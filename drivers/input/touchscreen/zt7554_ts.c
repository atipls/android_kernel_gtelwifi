...
static void run_preference_read(void *device_data)
{
	...
	for (i = 0; i < info->cap_info.x_node_num; i++) {
		for (j = 0; j < info->cap_info.y_node_num; j++) {
			dev_dbg(&info->client->dev, "pref_data[%d]: %d\n",
					i * info->cap_info.y_node_num + j,
...
static void get_preference(void *device_data)
{
	...
	node_num = x_node * info->cap_info.y_node_num + y_node;
	snprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "%u", raw_data->pref_data[node_num]);
	set_cmd_result(info, finfo->cmd_buff,
					strnlen(finfo->cmd_buff, sizeof(finfo->cmd_buff)));
	finfo->cmd_state = OK;
...
static void run_delta_read(void *device_data)
{
	...
	for (i = 0; i < info->cap_info.x_node_num; i++) {
		for (j = 0; j < info->cap_info.y_node_num; j++) {
			dev_dbg(&info->client->dev, "delta_data[%d]: %d\n",
...
static void get_delta(void *device_data)
{
	...
	node_num = x_node * info->cap_info.y_node_num + y_node;
	snprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "%u", raw_data->delta_data[node_num]);
	set_cmd_result(info, finfo->cmd_buff,
					strnlen(finfo->cmd_buff, sizeof(finfo->cmd_buff)));
	info->factory_info->cmd_state = OK;
...
static void run_ref_calibration(void *device_data)
{
	...
	dev_dbg(&info->client->dev, "%s: TSP calibration %s\n",
...
#if ESD_TIMER_INTERVAL
	write_reg(info->client, ZT7554_PERIODICAL_INTERRUPT_INTERVAL,
...
static ssize_t show_cmd_status(struct device *dev, struct device_attribute *devattr, char *buf)
{
	...
	dev_dbg(&info->client->dev, "tsp cmd: status:%d\n", finfo->cmd_state);
...
static ssize_t show_cmd_result(struct device *dev, struct device_attribute *devattr, char *buf)
{
	...
	dev_dbg(&info->client->dev, "tsp cmd: result: %s\n", finfo->cmd_result);
...
static void __init zt7554_ts_init_async(void *dummy, async_cookie_t cookie)
{
	i2c_add_driver(&zt7554_ts_driver);
	return;
}

static int __init zt7554_ts_init(void)
{
	async_schedule(zt7554_ts_init_async, NULL);
	return 0;
}
